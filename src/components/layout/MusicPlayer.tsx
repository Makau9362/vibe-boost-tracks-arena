    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-black border-t border-gray-800 px-2 sm:px-4 py-2 sm:py-3">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 w-1/3 sm:w-1/4 min-w-0">
          <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-md bg-gray-700 overflow-hidden flex-shrink-0">
            <img 
          ...
          <div className="hidden sm:block min-w-0">
            <p className="text-xs sm:text-sm font-medium truncate w-24 sm:w-36">{track.title}</p>
            <p className="text-xs text-gray-400 truncate w-24 sm:w-36">{track.artistName}</p>
          </div>
          <Button
            variant="ghost"
            size="icon"
            className="text-gray-400 hover:text-white h-6 w-6 sm:h-8 sm:w-8"
            onClick={toggleLike}
          >
            <Heart className={`h-3 w-3 sm:h-4 sm:w-4 ${isLiked ? 'fill-music-purple text-music-purple' : ''}`} />
          </Button>
        </div>
        ...
        <div className="flex flex-col items-center justify-center flex-1 max-w-xs sm:max-w-md">
          <div className="flex items-center justify-center space-x-1 sm:space-x-2 mb-1">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="bg-white text-black hover:bg-white/90 rounded-full h-6 w-6 sm:h-8 sm:w-8"
          ...
          <div className="w-full flex items-center space-x-1 sm:space-x-2">
            <span className="text-xs text-gray-400 w-6 sm:w-8 text-right">
            ...
            <span className="text-xs text-gray-400 w-6 sm:w-8">
              {formatTime(duration)}
            </span>
          </div>
        </div>
